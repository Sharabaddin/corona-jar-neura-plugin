...
#if !defined(LUA_ANSI) && (defined(_WIN32) || defined(WINAPI_FAMILY))
	#define LUA_WIN
	#if defined(WINAPI_FAMILY) && (WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP)
		#define LUA_WIN_PHONE
	#endif
#endif
...
/* Note: Not using POSIX because OBJC_EXCEPTIONS are used instead of ULONGJMP */
#if defined(LUA_USE_MODERN_MACOSX)
...
#else
#include "../../../librtt/Core/Rtt_Assert.h"
#define luai_apicheck(L,o)	{ (void)L; Rtt_ASSERT(o); }
#endif
#else
...
*/
/* C99 provides the exact size typedefs we need in the most correct and portable way possible.
 * Visual Studio finally got this in 2010.
 */
...
*/
	...
	#ifndef fopen
		#define fopen(filename, mode) lua_wfopen(filename, mode)
	#endif
	LUA_API FILE* lua_wfreopen(const char *filename, const char *mode, FILE *stream);
	#ifndef freopen
		#define freopen(filename, mode, stream) lua_wfreopen(filename, mode, stream)
	#endif
	LUA_API int lua_wremove(const char *filename);
	#ifndef remove
		#define remove(filename) lua_wremove(filename)
	#endif
	LUA_API int lua_wrename(const char *oldFilename, const char *newFilename);
	#ifndef rename
		#define rename(oldFilename, newFilename) lua_wrename(oldFilename, newFilename)
	#endif
...
#ifdef LUA_WIN_PHONE
	#ifndef LoadLibraryW
		#define LoadLibraryW(lpFileName) LoadPackagedLibrary(lpFileName, 0)
	#endif
	LUA_API char* lua_getenv(const char *varname);
	#ifndef getenv
		#define getenv(varname) lua_getenv(varname)
	#endif
#endif
